...
using PrachiIndia.Portal.Areas.Marketing.Models;
...
namespace PrachiIndia.Portal.Areas.Marketing.Controllers
{
    public class MarketingController : Controller
    {
        ...
        public ActionResult Index(string serach = "")
        {
            ...
            {
                scholIds = (from item in context.EbookOrders.Where(t => t.MEmail.ToLower() == username).ToList()
                            select item.UserID).ToList();
            }
            else
            {
                scholIds = (from item in context.EbookOrders.Where(t => t.MEmail.ToLower() == username && t.SchoolName.ToLower().Contains(serach)).ToList()
                            select item.UserID).ToList();
            }
            var results = (from asp in context.AspNetUsers.Where(t => scholIds.Contains(t.Id)).ToList()
                           select new MarketingModel
                           {
        ...
        public ActionResult ManageSchool(string Id)
        {
            ...
            if (aspnetUser == null)
                return RedirectToAction("Index");
            var appUser = new MarketingModel
            {
        ...
        [HttpPost]
        public ActionResult ManageSchool(MarketingModel model)
        {
        ...
        public FileResult DownloadFile(string RequestID, string UserType = "")
        {
            ...
            Response.AppendHeader("content-disposition", "attachment; filename=" + RequestID + ".zip");
